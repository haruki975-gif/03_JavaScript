...
function classTest(){
  ...
  */
  ...
  // class속성값이 "cls-test"인 요소를 모두 얻어와
  // (유사)배열 형태로 반환
  const divs = document.getElementsByClassName("cls-test");
  console.log(divs);
  
  // divs에 저장된 요소를 하나씩 순차 접근(순서대로, 차례대로)
...
function classTest2(){
  ...
  // "cls-input" class를 가지는 요소를 모두 얻어와 저장
...
function nameTest(){
  ...
  content += `<br><br>선택된 취미 개수 : ${count}개`;
...
function cssTest(){
  ...
  const colors = ['yellow', 'pink']; // 배열 선언 및 초기화(처음 값 대입)
  for(let i=0; i<divs.length; i++){
...
function readValue(){
  ...
  // * string(문자열).trim() : 문자열 좌우 공백을 제거
  if(input.value.trim().length === 0){ // 길이가 0 = 입력된 값이 없다
    alert("채팅 내용을 입력해 주세요.");
    input.value = ""; // 입력된 공백을 모두 제거
    input.focus(); // input 요소에 초점(포커스) 맞춤
    return;
  }
