...
startBtn.addEventListener("click", ()=>{
  ...
  if(startBtn.innerText === "PAUSE"){
    ...
    recordBtn.disabled = true; // 다시 비활성화
...
resetBtn.addEventListener("click", () => {
  startBtn.innerText = "START"; // START 버튼로 변경
  recordBtn.disabled = true; // 비활성화
  count = 0; // 시간 카운트 변수 0 초기화
...
function displayOutput(){
  ...
  // 화면에 출력된 시간 분, 초, 1/100초 계산 후 변수에 저장
  const minute = attachZero(Math.floor(count / 100 / 60)); // 몫 = 분
  const second = attachZero(Math.floor(count / 100 % 60)); // 나머지 = 초 또는 ms
...
recordBtn.addEventListener("click", ()=>{
  ...
  // console.log(display.innerText); -> 00 : 00 . 46
