...
function selectMenu(){
  const menus = [
    '김밥', '돈까스', '쫄면', '된장찌개', 
    '칼국수', '쌀국수', '카레', '닭갈비', '샌드위치',
    '샐러드', '라면', '제육덮밥', '짬뽕', '고등어 구이',
    '삼겹살', '백반', '우육면', '곰탕', '부대찌개', '텐동'
  ];
  
  // 0 이상 메뉴 개수 미만의 난수 1개 생성
  const num = Math.floor(Math.random() * menus.length);
  
  // id가 menuResult인 요소에 내용으로 출력
...
function createLottoNumber(){
  ...
  for(let i=0; i<6; i++){
    ...
    if(arr.includes(num)){
      i--; // index를 뒤로 갔다가 앞으로 오게 함(제자리 유지, 중복으로 빈값 해결)
  ...
  // 배열 내 숫자 정렬(sort)
  arr.sort( function(a,b){ return a-b; } ); // 오름차순(숫자가 커짐) 정렬
...
/** 2차원 배열 확인 2 */
/* 
  4행 4열짜리 2차원 배열에 1~16 사이 난수 배치 (중복X)
  1)  배열을 콘솔에 출력
...
function check7(){
  ...
  for(let row=0; row<4; row++){ // 행 반복
    ...
    for(let col=0; col<4; col++){ // 열 반복
      ...
      index++; // index 1 증가시켜 randomArr 다음 요소 지정하기
      
      // arr[row][col] = randomArr[index++];  // index++ 후위연산
    }
  }
  console.log(arr);
  ...
  // 2차원 배열 어디에 위치하는지 검색해서 출력하기
  for(let row = 0; row < arr.length; row++){ // 2차원 배열의 행 반복 (4)
    for(let col = 0; col < arr[row].length; col++){ // 열 반복 (4)
      
      // arr[row][col]에 저장된 값과 입력값(input)이 같을 때
