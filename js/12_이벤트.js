...
/*
  "변경하기" 버튼 클릭 시
  input 요소에 작성된 색상 값을 읽어 얻어와
  바로 위 .display요소의 배경색 변경하기
*/
const displays = document.querySelectorAll(".display"); // 배경 변경
const inputs = document.querySelectorAll(".inputColor"); // 색상 입력
const button = document.getElementById("changeBtn"); // 변경하기 버튼
...
button.addEventListener("click", function(){
  ...
  for(let i = 0; i < inputs.length; i++){
    ...
    // input칸도 색 채우기
    // inputs[i].style.backgroundColor = inputs[i].value;
    
    // div > display부분에 입력한 색 채우기
