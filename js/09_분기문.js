...
function check6(){
	...
	// -> 문자열 내에 검색어를 찾아서 바꿀 단어로 변경
	for(let num = 1; num <= 5; num++){
		let str = "123456789";
		
		// num과 일치하는 숫자를 str에서 찾아서 ''빈칸으로 변경
		str = str.replace(num, '');
		console.log(str);
	}
}
...
/** UP & DOWN 게임 */
/* 
	1부터 200 사이 난수를 몇 회만에 맞추는지 카운트하는 게임
		
	조건 1. prompt를 이용해서 숫자 입력 받기
		- 미입력 경우 :
			alert("다시 입력해 주세요."); - (카운트 증가 X)
	...
	조건 3. 입력 받은 수가 난수(answer)보다 높거나 낮을 경우
		- 높은 경우 : alert(`DOWN (현재 ${count}회)`);
		- 낮은 경우 : alert(`UP (현재 ${count}회)`);
...
function startGame(){
	...
	// 맞춰야 하는 정답(1 ~ 200 사이) 생성
	const answer = Math.floor(Math.random() * 200);
	console.log("정답 : ", answer); // test 후에 삭제
