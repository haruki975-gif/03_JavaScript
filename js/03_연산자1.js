...
/* Number(input1.value)가 계속 반복되는데 전역 변수로 선언하면 안될까?
    ...
    왜? 전역 변수는 페이지 로딩 시점에 해석돼서 값이 결정된다!
    -> input애 작성된 값이 없어서 Number(input1.value) == 0
    -> (해결방법) 값을 입력한 후 함수를 호출해서
    함수가 호출되는 시점의
    입력된 값을 읽어올 수 있게 된다!!!'
*/
...
function allOperationFn(){
    ...
    "<hr><hr><ul style='list-style:none;'>" 
    + "<li> + 결과 : " + plus + "</li>"
    + "<li> - 결과 : " + minus + "</li>"
    + "<li> * 결과 : " + multi + "</li>"
    + "<li> / 결과 : " + div + "</li>"
...
// ex) +, -, *, /, %, ++, --,
//    변수에 값 대입, console 출력, innerText... 모두 다 연산의 대상
